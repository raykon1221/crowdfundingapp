...
export default function DashboardPage() {
    ...
    // Get Campaigns getUserCampaign
    const { data: myCampaigns, isPending: isPendingMyCampaigns, refetch } = useReadContract({
        contract: contract,
        method: "function getUserCampaign(address _user) view returns ((address campaignAddress, address owner, string name, uint256 creationTime)[])",
...
const CreateCampaignModal = ({ setIsModalOpen, refetch }: CreateCampaignModalProps) => {
    ...
    // function to Deploy contract from CrowdfundingFactory
    const handleDeployContract = async () => {
        setIsDeployingContract(true);
        try {
            console.log("Deploying contract...");
            const contractAddress = await deployPublishedContract({ // deploy published contract
                client: client,
                chain: sepolia,
                account: account!,
                ...
                publisher: "0x163ac1ccAa7e63Ed01D1fb90c0cc32FB676ACEDd",
                version: "1.0.0",
            });
    ...
    //minimum Campaign goal has to be 1
